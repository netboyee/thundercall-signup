...
var thunderCallSignupPath = "/api/users/signup";
var signatureHeader = "X-Thundercall-Signup-Signature";
var timestampHeader = "X-Thundercall-Signup-Timestamp";
var clientIPHeader = "X-Thundercall-Client-IP";
...
async function onRequestPost(context2) {
  ...
  const apiBaseURL = trimTrailingSlash(env2.THUNDERCALL_API_BASE_URL || "https://api.thundercall.com");
  const sharedSecret = String(env2.THUNDERCALL_API_PUBLIC_SIGNUP_PROXY_SHARED_SECRET || "").trim();
  if (sharedSecret === "") {
    return jsonResponse(500, { message: "ThunderCall signup proxy is not configured." });
  }
  const clientIP = forwardedClientIP(request);
  const turnstileSecret = String(env2.TURNSTILE_SECRET_KEY || "").trim();
  if (turnstileSecret !== "") {
...
__name(computeProxySignature, "computeProxySignature");
...
__name(forwardedClientIP, "forwardedClientIP");
...
// ../.wrangler/tmp/pages-YgwFiZ/functionsRoutes-0.25521619746422464.mjs
var routes = [
  {
